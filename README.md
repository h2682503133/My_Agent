...
1. 实现通过差异化智能体调用来切分智能体所加载的系统提示词与任务中的记忆，以此降低token消耗
...
# 下载后必须要做的事
1. 使用environment.yml文件完成依赖下载
2. 修改config文件夹内文件(特别是model_list，原文件已废置，请删除并修改example进行配置)
# 文件结构介绍
...
6. viking_data - viking知识库目录，无需手动配置
# 实现介绍
...
3. 理想状态下的工作流程，1.main对用户任务进行拆解，2.main将某步的需求交给tool调用工具进行完成(tool会根据调用情况自行调整)，tool将完成情况返回main，重复2，3直到任务完成，reader对工具调用过程和智能体调度过程进行总结，记录在知识库中
但在实际过程中，由于用户需求对tool可见导致tool想直接自行完成任务
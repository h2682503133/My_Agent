...
def queue_and_retry(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        gateway_log(f'{ip} - - [{now}] "{method} {path} {request.environ.get("SERVER_PROTOCOL", "HTTP/1.1")}" - 请求已接收，进入队列')
        ...
        for retry_idx in range(MAX_RETRY):
            try:
                ...
                if t.is_alive():
                    ...
                    gateway_log(f"[超时失败] 用户 {user_id} 第{retry_idx+1}次，重试")
                    continue
